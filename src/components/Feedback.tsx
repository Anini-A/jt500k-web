...
export function useConfirm() {
  ...
  const confirmNode = p && typeof document !== 'undefined' ? createPortal(
    <div onClick={() => setP(null)} style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(0,0,0,0.45)', WebkitBackdropFilter: 'blur(3px)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: 18, width: '100%', maxWidth: 340, boxShadow: '0 14px 40px rgba(0,0,0,0.38)' }}>
...
export function useToast() {
  ...
  const toastNode = msg && typeof document !== 'undefined' ? createPortal(
    <div style={{ position: 'fixed', left: '50%', bottom: 'calc(96px + env(safe-area-inset-bottom, 0px))', transform: 'translateX(-50%)', zIndex: 320, background: 'var(--text-primary)', color: 'var(--surface-1)', padding: '10px 18px', borderRadius: 'var(--radius-pill)', fontSize: 'var(--fs-sm)', fontWeight: 600, boxShadow: '0 6px 20px rgba(0,0,0,0.3)', maxWidth: '90vw', textAlign: 'center' }}>{msg}</div>,
    document.body) : null

// Estimate net-worth history for the months BEFORE any real data exists
// (i.e. before the earliest real snapshot, back to the first transaction).
// Cash & debts are reconstructed from real transactions; only investment growth
// is estimated via REAL monthly index returns, anchored to the earliest real
// value so the estimate converges to the truth there.
...
export function reconstructHistory(opts: {
  ...
  if (monthsApart(startMonth, anchorMonth) <= 0) return []
  ...
  for (let m = anchorMonth; m > startMonth; m = addMonth(m, -1)) {
    ...
    out.push({ month: addMonth(m, -1), net: Math.round(holdings + cash - debts), est: true })
  }
  return out.reverse()
}

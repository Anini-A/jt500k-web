...
export function useToast() {
  ...
  const toastNode = msg && typeof document !== 'undefined' ? createPortal(
    <div style={{ position: 'fixed', left: '50%', bottom: 'calc(96px + env(safe-area-inset-bottom, 0px))', transform: 'translateX(-50%)', zIndex: 320, background: 'var(--text-primary)', color: 'var(--surface-1)', padding: '10px 18px', borderRadius: 999, fontSize: 'var(--fs-sm)', fontWeight: 600, boxShadow: '0 6px 20px rgba(0,0,0,0.3)', maxWidth: '90vw', textAlign: 'center' }}>{msg}</div>,
    document.body) : null
